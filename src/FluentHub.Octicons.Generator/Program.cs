...
namespace FluentHub.Octicons.Generator
{
	...
	public class Program
	{
		...
		private static string ConvertSvgEntity(string path)
		{
			...
			var pathElement = doc.GetElementsByTagName("path")
				.OfType<XmlElement>()
				.FirstOrDefault() ?? throw new InvalidDataException($"SVG path element was not found: {path}");

			var svgRaw = pathElement.GetAttribute("d");
			if (string.IsNullOrEmpty(svgRaw))
				throw new InvalidDataException($"SVG path data was not found: {path}");
